...
`effort:` behaves differently: a skill whose body prints `${CLAUDE_EFFORT}` returned `low` with `effort: low` in every path (inline direct, inline auto, fork) and `xhigh` — the `settings.json` `effortLevel` — without the field. This shows Claude Code applies the override to its own effort state; the API request parameter itself is not visible in stream-json or the `--debug` log, so that last step is inferred.
