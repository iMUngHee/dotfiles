...
| No `make`, so no `make test` / `make reinstall` for pager | `make` is not installed, and installing it would not bring those targets back: `test` runs `go test -race`, `-race` needs cgo, and the Makefile exports `CGO_ENABLED=0` precisely so no C toolchain is ever required. Race is out of reach here anyway — TSan references `WakeByAddressSingle`/`WakeByAddressAll`, and zig's bundled MinGW ships no `libsynchronization` to resolve them from. `scripts/install-pager.ps1` covers `make build`; the gate that does run is `go test ./...` without `-race`, which passes in full. |
| `git status` shows every tracked file modified | A clone made before `.gitattributes` landed keeps index stat entries sized for the CRLF working tree, so every file reads as modified while `git diff` is empty and `git merge --ff-only` refuses to run. `git add --renormalize .` clears it: the blobs already match, so nothing is staged. Hit in both this repo and `pager`. |
